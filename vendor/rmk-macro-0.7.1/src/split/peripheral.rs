...
fn expand_bind_interrupt_for_split_peripheral(chip: &ChipModel, communication: &CommunicationConfig) -> TokenStream2 {
    match chip.series {
        ChipSeries::Nrf52 => {
            ...
            let clock_power_handlers = if communication.usb_enabled() {
                quote! {
            ...
            let usb_interrupt = communication
                .get_usb_info()
                .map(|usb_info| {
...
fn expand_split_peripheral(
    ...
    // `rmk_peripheral` historically initialized BLE and the matrix only. Keep
    // normal peripheral firmware unchanged, but let this project opt into a
    // CDC logger for the explicit usb-debug profile.
    let usb_debug_enabled = keyboard_config
        .get_communication_config()
        .unwrap()
        .usb_enabled();
    let usb_init = if usb_debug_enabled {
...
fn expand_split_peripheral_entry(
    ...
    if split_config.connection == "ble" {
        ...
        let run_without_usb = join_all_tasks(tasks.clone());
        if usb_debug_enabled {
            tasks.push(quote! { ::rmk::run_usb_logger(driver, "vial:f64c2b3c:peripheral") });
            let run_with_usb = join_all_tasks(tasks);
            quote! {
                #[cfg(feature = "usb-debug")]
                {
                    #run_with_usb
        ...
        tasks.extend(controllers);
        let run_without_usb = join_all_tasks(tasks.clone());
        let run_rmk_peripheral = if usb_debug_enabled {
            tasks.push(quote! { ::rmk::run_usb_logger(driver, "vial:f64c2b3c:peripheral") });
            let run_with_usb = join_all_tasks(tasks);
            quote! {
                #[cfg(feature = "usb-debug")]
